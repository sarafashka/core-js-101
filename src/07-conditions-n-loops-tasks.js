...
function doRectanglesOverlap(rect1, rect2) {
  const down1 = rect1.top + rect1.height;
  const right1 = rect1.left + rect1.width;

  const top2 = rect2.top;
  const left2 = rect2.left;

  return left2 <= right1 && top2 <= down1;
}
...
function findFirstSingleChar(str) {
  ...
  strnew.split('').forEach((char) => {
    const result = str.split('').filter((item) => item === char);
    if (result.length === 1 && singleChar === null) {
      singleChar = char;
    }
  });
  return singleChar;
...
function getDigitalRoot(num) {
  ...
  while (arr.length > 1) {
    const sum = arr.reduce((accum, item) => Number(accum) + Number(item));
